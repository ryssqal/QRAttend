...
<head>
    ...
    <title>Edit Event - QRAttend</title>
    <!-- Fonts -->
...
<body class="antialiased bg-gray-50">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <!-- Form -->
        <form action="{{ route('pengurusMajlis.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            @csrf
            @method('PUT')
            ...
            @if($event->media_path)
                <div class="mb-6">
                    ...
                    <div class="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                        @if(strpos($event->media_path, '.mp4') !== false || strpos($event->media_path, '.mov') !== false)
                            <video class="w-20 h-20 object-cover rounded-lg" controls>
                                <source src="{{ Storage::url($event->media_path) }}" type="video/mp4">
                            </video>
                        @else
                            <img src="{{ Storage::url($event->media_path) }}" alt="Current media" class="w-20 h-20 object-cover rounded-lg">
                        @endif
                        <p class="text-sm text-gray-600">Leave the upload field empty to keep current media</p>
                    </div>
                </div>
            @endif
            ...
            <div class="mb-6">
                <label for="media" class="block text-sm font-medium text-gray-700 mb-2">
                    Upload Photo / Video <span class="text-gray-500">(Optional - leave empty to keep current)</span>
                </label>
            ...
            <div class="mb-6">
                ...
                <div class="mt-1 flex justify-between">
                    ...
                    <p class="text-sm text-gray-500"><span id="char-count">{{ strlen(old('description', $event->description)) }}</span>/1000</p>
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" id="pax" name="pax" value="{{ old('pax', $event->pax) }}" min="1" max="10000"
            ...
            <div class="mb-6">
                ...
                <input type="password" id="password" name="password" value="{{ old('password') }}"
                       ...
                       placeholder="Leave empty for public event or to keep current password">
                <p class="mt-1 text-sm text-gray-500">If set, event becomes private. 4-20 characters. Leave empty to keep current password.</p>
            </div>

            <!-- Submit Button -->
            <div class="flex justify-end space-x-4">
                <a href="{{ route('pengurusMajlis.events.index') }}"
                ...
                <button type="submit" id="submit-btn"
                    ...
                    <span id="submit-text">Update Event</span>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            submitText.textContent = 'Updating Event...';
